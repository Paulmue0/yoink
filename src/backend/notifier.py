...
class Notifier:
    ...
    def send_all_notifications(self, type: Condition = Condition.NEW):
        ...
        for product in products:
            to_be_updated_item = self.store.find_item_by_name(
    ...
    def get_all_unsent_messages(self, chat_id: str) -> list[Message]:
        ...
        messages = []
        for product in products:
            to_be_updated_item: Result = self.store.find_item_by_name(
                product['name'], [QueryType.RESULTS, Condition.NEW.value])
            if not to_be_updated_item.sent:
